...
<template>
    <GlobalCard>
        <p class="font-Mohave text-2xl font-semibold uppercase">{{ t("home_emergencyBreakdown") }}</p>

        <div class="mt-4 flex w-full justify-center">
            <apexchart height="320" type="donut" :options="chartOptions" :series="chartSeries"></apexchart>
        </div>

        <div class="mt-4 flex justify-center border-t border-t-gray-200 p-4 dark:border-t-gray-700">
            <div class="flex flex-wrap justify-center gap-4 xl:w-2/3">
                <div v-for="(stat, key) in userStore.user.clientStats.missions" :key="key" class="flex items-center justify-center">
                    <div class="pi mr-2 h-3 w-3 rounded-full" :style="`background-color: ${getMissionColor(key)}`"></div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                        {{ getMissionType(key) }}: <span class="font-semibold text-gray-900 dark:text-white">{{ stat }}</span>
                    </div>
                </div>
            </div>
        </div>
