...
<template>
	<div v-if="emergencyStore.trackedEmergency">
		...
		<GlobalCard class="mt-8">
			<p
				class="
					font-Mohave text-3xl font-bold text-primary-600
					dark:text-red-700
				"
			>
			...
			<div v-if="emergencyStore.trackedEmergency.respondingTeam.staff.length > 0 && emergencyStore.trackedEmergencyTeamDetails">
				<p class="mt-8 font-Mohave text-2xl font-bold">
					{{ t("tracking_responders") }}
				</p>

				<GlobalTextBox
					v-if="getAssignedTeamsString || showSuccessRate" class="
						mt-2 text-sm
						lg:w-fit
					" font-weight="normal"
				>
					<div class="flex items-center">
						<p v-if="getAssignedTeamsString">
							{{ getAssignedTeamsString }}
						</p>
						<p v-if="getAssignedTeamsString && showSuccessRate" class="px-1 font-medium">
							|
						</p>
